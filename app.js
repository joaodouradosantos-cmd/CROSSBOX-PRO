...
/* ==================================================
   WOD – QUADRO COMPLETO (NOTAS LIVRES POR DIA)
   - Offline-first
   - Guarda texto por data
   ================================================== */
(function () {
  const STORAGE_WOD_BOARD_NOTES = "crossfit_wod_board_notes";

  const wodBoardText = document.getElementById("wodBoardText");
  const wodBoardTemplateBtn = document.getElementById("wodBoardTemplateBtn");
  const wodBoardClearBtn = document.getElementById("wodBoardClearBtn");

  if (!wodBoardText) return;
  ...
  function loadNotes() {
    const raw = localStorage.getItem(STORAGE_WOD_BOARD_NOTES);
  ...
  function saveNotes(obj) {
    try {
      localStorage.setItem(STORAGE_WOD_BOARD_NOTES, JSON.stringify(obj || {}));
    } catch (e) {
      console.warn("Falha ao guardar quadro do dia:", e);
    }
  }

  function getDiaAtual() {
    const hoje = new Date().toISOString().slice(0, 10);
    return (typeof treinoDataEl !== "undefined" && treinoDataEl && treinoDataEl.value) ? treinoDataEl.value : hoje;
  }

  function render() {
    const dia = getDiaAtual();
    const notes = loadNotes();
    wodBoardText.value = (notes[dia] != null) ? String(notes[dia]) : "";
  }
  ...
  function scheduleSave() {
    ...
    saveT = setTimeout(() => {
      const dia = getDiaAtual();
      const notes = loadNotes();
      const txt = String(wodBoardText.value || "").trimEnd();

      if (txt) notes[dia] = txt;
      else delete notes[dia];

      saveNotes(notes);
    }, 250);
  }

  wodBoardText.addEventListener("input", scheduleSave);

  if (typeof treinoDataEl !== "undefined" && treinoDataEl) {
    treinoDataEl.addEventListener("change", () => {
      // antes de trocar de dia, força save do que estava a escrever
      scheduleSave();
      setTimeout(render, 0);
    });
  }

  if (wodBoardTemplateBtn) {
    wodBoardTemplateBtn.addEventListener("click", () => {
      const tpl =
`A) 
  - 
...
D) 
  - 
`;
      // se estiver vazio, mete template; se já tiver texto, acrescenta no fim
      if (!wodBoardText.value.trim()) {
        wodBoardText.value = tpl;
      } else {
        const sep = wodBoardText.value.endsWith("\n") ? "\n" : "\n\n";
        wodBoardText.value = wodBoardText.value + sep + tpl;
      }
      wodBoardText.focus();
      scheduleSave();
    });
  ...
  if (wodBoardClearBtn) {
    wodBoardClearBtn.addEventListener("click", () => {
      wodBoardText.value = "";
      scheduleSave();
      wodBoardText.focus();
    });
