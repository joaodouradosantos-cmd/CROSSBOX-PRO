...
function exportHistoricoPdf() {
  try {
    ...
    let html = '<h1>Histórico WOD</h1>';
    html += '<p>Gerado em ' + new Date().toLocaleString("pt-PT") + '</p>';
    html += '<table border="1" cellspacing="0" cellpadding="6"><tr><th>Data</th><th>Parte</th><th>Exercício</th><th>Reps</th><th>Peso</th></tr>';
    treinos.forEach(t => {
      if (!t) return;
      html += '<tr>' +
        '<td>' + (t.date||'') + '</td>' +
