...
const STORAGE_WOD_QUADRO = "crossfit_wod_quadro_v1";
...
const openQuadroBoxBtn = document.getElementById("openQuadroBoxBtn");
const quadroBoxStatusEl = document.getElementById("quadroBoxStatus");
const quadroBoxModalEl = document.getElementById("quadroBoxModal");
const closeQuadroBoxBtn = document.getElementById("closeQuadroBoxBtn");
const quadroBoxTextEl = document.getElementById("quadroBoxText");
const quadroBoxTemplateBtn = document.getElementById("quadroBoxTemplateBtn");
const quadroBoxClearBtn = document.getElementById("quadroBoxClearBtn");
const quadroBoxSaveBtn = document.getElementById("quadroBoxSaveBtn");
const quadroBoxSaveStatusEl = document.getElementById("quadroBoxSaveStatus");
...
let wodQuadro = {};
try {
  const raw = localStorage.getItem(STORAGE_WOD_QUADRO);
  wodQuadro = raw ? JSON.parse(raw) : {};
  if (!wodQuadro || typeof wodQuadro !== "object" || Array.isArray(wodQuadro)) wodQuadro = {};
} catch (e) {
  wodQuadro = {};
}

function saveWodQuadro() {
  try { localStorage.setItem(STORAGE_WOD_QUADRO, JSON.stringify(wodQuadro)); } catch(e) {}
}

function getDiaAtual() {
  return (treinoDataEl && treinoDataEl.value) ? treinoDataEl.value : new Date().toISOString().slice(0, 10);
}

function updateQuadroBoxStatus() {
  if (!quadroBoxStatusEl) return;
  const dia = getDiaAtual();
  const has = Boolean((wodQuadro[dia] || "").trim());
  quadroBoxStatusEl.textContent = has ? "Guardado ✅" : "Sem quadro";
}

function openQuadroBoxModal() {
  if (!quadroBoxModalEl || !quadroBoxTextEl) return;
  const dia = getDiaAtual();
  quadroBoxTextEl.value = wodQuadro[dia] || "";
  quadroBoxModalEl.classList.remove("hide");
  // impedir scroll do fundo
  document.body.style.overflow = "hidden";
  setTimeout(() => { try { quadroBoxTextEl.focus(); } catch(e) {} }, 50);
  updateQuadroBoxStatus();
}

function closeQuadroBoxModal() {
  if (!quadroBoxModalEl) return;
  quadroBoxModalEl.classList.add("hide");
  document.body.style.overflow = "";
  updateQuadroBoxStatus();
}

function setQuadroBoxText(text) {
  const dia = getDiaAtual();
  const val = String(text || "");
  if (val.trim()) wodQuadro[dia] = val;
  else delete wodQuadro[dia];
  saveWodQuadro();
  updateQuadroBoxStatus();
}
...
/* Quadro da box – UI */
if (openQuadroBoxBtn) {
  openQuadroBoxBtn.addEventListener("click", openQuadroBoxModal);
}
if (closeQuadroBoxBtn) {
  closeQuadroBoxBtn.addEventListener("click", closeQuadroBoxModal);
}
if (quadroBoxModalEl) {
  // clique fora do cartão fecha
  quadroBoxModalEl.addEventListener("click", (e) => {
    if (e.target === quadroBoxModalEl) closeQuadroBoxModal();
  });
  // ESC fecha
  document.addEventListener("keydown", (e) => {
    if (e.key === "Escape" && !quadroBoxModalEl.classList.contains("hide")) closeQuadroBoxModal();
  });
}
const quadroBoxSaveBtn = document.getElementById("quadroBoxSaveBtn");
const quadroBoxClearBtn = document.getElementById("quadroBoxClearBtn");
...
let quadroDebounce = null;
function setSaveStatus(msg) {
  if (quadroBoxSaveStatusEl) quadroBoxSaveStatusEl.textContent = msg || "";
}

if (quadroBoxTextEl) {
  quadroBoxTextEl.addEventListener("input", () => {
    setSaveStatus("A guardar…");
    clearTimeout(quadroDebounce);
    quadroDebounce = setTimeout(() => {
      setQuadroBoxText(quadroBoxTextEl.value);
...
if (quadroBoxSaveBtn) {
  quadroBoxSaveBtn.addEventListener("click", () => {
    if (!quadroBoxTextEl) return;
    setQuadroBoxText(quadroBoxTextEl.value);
    setSaveStatus("Guardado ✅");
  });
}

if (quadroBoxClearBtn) {
  quadroBoxClearBtn.addEventListener("click", () => {
    if (!confirm("Limpar o quadro deste dia?")) return;
    if (quadroBoxTextEl) quadroBoxTextEl.value = "";
    setQuadroBoxText("");
    setSaveStatus("Limpo.");
  });
}

if (quadroBoxTemplateBtn && quadroBoxTextEl) {
  quadroBoxTemplateBtn.addEventListener("click", () => {
    const tpl = "A)\n\nB)\n\nC)\n\nD)\n";
    if (quadroBoxTextEl.value && !confirm("Substituir o texto atual pelo modelo?")) return;
    quadroBoxTextEl.value = tpl;
    setQuadroBoxText(tpl);
    setSaveStatus("Guardado ✅");
  });
}
