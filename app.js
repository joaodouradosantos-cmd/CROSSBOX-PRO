...
// (Removido) Resultado global do WOD (RX/adaptado/incompleto)
// Limpa eventual armazenamento antigo para não deixar lixo.
...
// (Removido) Resultado global do WOD
...
// (Removido) saveWodResultados / updateResultadoUIForDate
...
if (treinoDataEl) {
  treinoDataEl.addEventListener("change", () => {
    // O registo (Resumo) é sempre do dia selecionado.
    // Ao mudar de dia, fechamos o painel para não ficar "aberto" vazio
    // e evitamos que o layout cresça para baixo.
    if (wodResumoDetailsEl) wodResumoDetailsEl.open = false;
    renderTreinos();
  });
}

// Ao arrancar, deixamos o Resumo fechado por defeito.
// (o utilizador abre quando quiser)
if (wodResumoDetailsEl) {
  wodResumoDetailsEl.open = false;
}
