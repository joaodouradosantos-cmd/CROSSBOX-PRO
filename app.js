...
document.querySelectorAll(".tab-btn").forEach(btn => {
  btn.addEventListener("click", () => {
    ...
    // === Forçar o modo correto por tab (evita WOD/TOOLS trocados) ===
    // WOD: garantir que fica ativa a sub-secção opt-wod
    if (target === "sec-wod") {
      document.querySelectorAll(".option-section").forEach(s => s.classList.remove("active"));
      const wodSec = document.getElementById("opt-wod");
      if (wodSec) wodSec.classList.add("active");
      try { localStorage.setItem("crossbox_last_tab", "wod"); } catch(e) {}
    }

    // TOOLS: abrir por defeito o histórico (ou última ferramenta usada)
    if (target === "sec-tools") {
      document.querySelectorAll(".option-section").forEach(s => s.classList.remove("active"));
      let lastTools = null;
      try { lastTools = localStorage.getItem("crossbox_last_tool_opt"); } catch(e) {}
      const wanted = lastTools || "opt-historico";
      const toolSec = document.getElementById(wanted) || document.getElementById("opt-historico");
      if (toolSec) toolSec.classList.add("active");
      try { localStorage.setItem("crossbox_last_tab", "tools"); } catch(e) {}
    }

    if (target === "sec-1rm") {
      try { localStorage.setItem("crossbox_last_tab", "1rm"); } catch(e) {}
    }
  });
});
/* ---------------------------------------------------
