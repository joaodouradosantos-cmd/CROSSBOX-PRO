...
(function(){
  ...
  const saveForDay = () => {
    ...
    // ✅ sincronizar com o HISTÓRICO (crossfit_treinos)
    try {
      const raw = localStorage.getItem("crossfit_treinos") || "[]";
      let arr = JSON.parse(raw);
      if (!Array.isArray(arr)) arr = [];

      // remover qualquer registo "QUADRO" existente para esta data
      arr = arr.filter(e => !((e && e.date === k) && (e.tipo === "QUADRO")));
      ...
      if (val) {
        arr.unshift({
          ...
          reps: 0,
          peso: 0,
          tempo: val,            // mostramos o texto na coluna "Tempo"
          carga: null,
          perc1rm: null,
          distanciaKm: 0
        });
      ...
      localStorage.setItem("crossfit_treinos", JSON.stringify(arr));
      // manter variável global "treinos" consistente, se existir
      if (typeof treinos !== "undefined") treinos = arr;
      ...
      // não bloqueia o utilizador se o storage estiver corrompido
      console.warn("Falha ao sincronizar Quadro com Histórico:", e);
    }
...
`;
  ...
  dateEl.addEventListener("change", () => setStatus(""));
})();
