...
// ─── CONSTANTES ──────────────────────────────────────────────
// PIN do professor removido do código — autenticação verifica token no Firestore
// Configuração: Firebase Console → Firestore → colecção "config" → documento "professor"
// Adicionar campo:  token  (string)  →  valor: uma string longa e aleatória (≥32 chars)
let _profTokenCache = null;

async function verificarTokenProf(tokenIntroduzido) {
  try {
    const snap = await getDoc(doc(db, "config", "professor"));
    if (!snap.exists()) return false;
    const tokenCorreto = snap.data().token || "";
    if (tokenCorreto.length < 8) return false;
    return tokenIntroduzido === tokenCorreto;
  } catch (e) {
    console.error("Erro ao verificar token professor:", e);
    return false;
  }
}
...
async function bloquearAluno(tel) {
  await updateDoc(doc(db,"alunos",tel),{bloqueado:true,_profToken:_profTokenCache||""});
  const hoje = isoDate(new Date());
  const snap = await getDocs(collection(db,"aulas"));
...
async function eliminarAluno(tel) {
  try {
    // Elimina todas as inscrições deste aluno nas aulas existentes
    const aulasSnap = await getDocs(collection(db,"aulas"));
    const batch = [];
    aulasSnap.forEach(d => {
      const a = d.data();
      const inscritos = (a.inscritos||[]).filter(x=>x.tel!==tel);
      if(inscritos.length !== (a.inscritos||[]).length) {
        batch.push(updateDoc(doc(db,"aulas",d.id),{inscritos,_profToken:_profTokenCache||""}));
      }
    });
    await Promise.all(batch);
    // Elimina o documento do aluno
    await deleteDoc(doc(db,"alunos",tel));
    return true;
  } catch(e) {
...
function renderGrid() {
  ...
  // Guardar qual dia estava aberto antes de re-render, sem forçar o dia de hoje
  const diaAbertoAntes = diaAbertoCalendario || grid.querySelector(".cal-dia-aulas.cal-dia-aulas-open")?.dataset.dt || null;
  ...
  for (let i=0;i<7;i++) {
    ...
    // Abrir apenas o dia escolhido pelo utilizador; não voltar automaticamente ao dia de hoje
    const deveAbrir = dt===diaAbertoAntes;
  ...
  grid.querySelectorAll(".cal-dia-toggle").forEach(btn=>{
    btn.addEventListener("click", e=>{
      ...
      diaAbertoCalendario = aberto ? dt : null;
...
function renderLogin() {
  ...
  document.getElementById("cal-prof").addEventListener("click",async()=>{
    const pin=prompt("🔐 PIN do professor:");
    if(pin===null)return;
    const btn=document.getElementById("cal-prof");
    if(btn){btn.disabled=true;btn.textContent="A verificar...";}
    const ok=await verificarTokenProf(pin);
    if(btn){btn.disabled=false;btn.textContent="🔐 ACESSO PROFESSOR";}
    if(ok){_profTokenCache=pin;saveSession({nome:"Professor",tel:"prof",tipo:"prof"});entrarNaApp();}
...
export async function renderAlunosPublic(div) {
  ...
  snap.forEach(d=>{
    ...
    html+=`<div class="cal-aluno-row${a.bloqueado?" cal-row-bloq":""}">
      ...
      <div style="display:flex;gap:6px;flex-wrap:wrap;">
        ...
        <button class="cal-btn-eliminar" data-tel="${a.tel}" data-nome="${a.nome}" style="background:rgba(180,30,30,0.18);color:#e07070;border:1px solid rgba(200,60,60,0.4);">🗑️ Eliminar</button>
      </div>
    </div>`;
  });
  div.innerHTML=html;
  ...
  div.querySelectorAll(".cal-btn-reativar").forEach(btn=>btn.addEventListener("click",async()=>{
    await updateDoc(doc(db,"alunos",btn.dataset.tel),{bloqueado:false,_profToken:_profTokenCache||""});await renderAlunosPublic(div);
  }));
  div.querySelectorAll(".cal-btn-eliminar").forEach(btn=>btn.addEventListener("click",async()=>{
    const nome=btn.dataset.nome||btn.dataset.tel;
    if(!confirm(`⚠️ Eliminar ${nome} permanentemente?

Esta acção remove o aluno e todas as suas inscrições. Não é reversível.`))return;
    btn.disabled=true;btn.textContent="A eliminar...";
    const ok=await eliminarAluno(btn.dataset.tel);
    if(ok){await renderAlunosPublic(div);}
    else{btn.disabled=false;btn.textContent="🗑️ Eliminar";alert("Erro ao eliminar. Verifica a ligação e tenta de novo.");}
  }));
}
